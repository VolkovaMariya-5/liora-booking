...
// /manage/bookings — все записи бизнеса для владельца (Phase 7)
export default async function ManageBookingsPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {STATUS_TABS.map((tab) => {
          const href = tab.value ? `/manage/bookings?status=${tab.value}` : '/manage/bookings';
          return (
            <Link
              key={tab.value}
              href={href}
              ...
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <p className="font-semibold text-foreground">
                    {b.clientName ?? 'Клиент'} → {b.staff?.name ?? 'Мастер'}
                  </p>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">
                    ...
                    в {b.startTime}
                    {b.totalPrice !== undefined && ` · ${Number(b.totalPrice).toLocaleString('ru')} ₽`}
                  </p>
                </div>
                <StatusBadge status={b.status as any} />
              </div>
              <ManageBookingActions bookingId={b.id} currentStatus={b.status} />
            </div>
          ))}
        </div>
      )}
